...
class Model_Csrf extends \Orm\Model
{
  ...
  public static function check() 
  {
    ...
    {
      $res[0] = 2;
      $res[1] = 'csrf id is wrong';
      Model_Log::warn('csrf id is wrong');
      die(json_encode($res));
    }
    ...
    {
      $res[0] = 2;
      $res[1] = 'csrf record is wrong';
      Model_Log::warn('csrf record is wrong');
      die(json_encode($res));
    }
    if ($csrf->token != Cookie::get('csrf'))
    {
      $res[0] = 2;
    ...
    //within 10 minutes, you must post
    DB::query("delete from csrf where id = ".Cookie::get('csrf_id').
      " OR create_at < "."'".date('Y-m-d H:i:s', time()+600)."'")->execute();
