...
class Controller_FbOAuth extends Controller
{
  public function action_index()
  {
    ...
    $login_url = 'https://www.facebook.com/dialog/oauth?'.$client_id.'redirect_uri=http://'.$_SERVER['HTTP_HOST'].'/fboauth/';
    ...
    foreach ($arr_check_usr as $k => $d) {
      ...
      if ($d['pv_u_id'] AND $d['provider']) {
        if ($id != $d['pv_u_id'] AND 1 != $d['provider']) {
          ++$dupli_auth;
        }
        ++$auth_usr_i;
      }
    ...
    if ($auth_usr_i < 1) {
      ...
      $usr->pv_u_id = $id;
      $usr->provider = 1;
      $usr->name = $myname = $contents->first_name;
