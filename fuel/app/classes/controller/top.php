...
class Controller_Top extends Controller
{
  ...
  {
    $view = View::forge('top');
    $res = DB::query("select count(*) from question")
      ->execute()->as_array();
  ...
  {
    $view = View::forge('top');
    $query = DB::query("SELECT question_id FROM answer_by_q WHERE create_at < NOW() ORDER BY".
            "(30 - EXTRACT( DAY FROM(NOW() - create_at) )) * amount DESC
