...
class Controller_AdminCheck extends Controller
{
  public function action_index()
  {
    ...
    $arr_read = array(0,0);
    $arr_write = array(0,0);
    $arr_freemem = array(0,0);
    $arr_cpu = array(0,0);
    if (($handle = @fopen(APPPATH.'classes/monitor/monitor_'.date('Ymd',strtotime('-1 day')).'.csv', "r")) !== FALSE) {
    ...
    $content = $mail;
    $mailfrom="From:" .mb_encode_mimeheader("クイズジェネレーター") ."<generator@mail.komahana.info>";
    $res = mb_send_mail($mailto,$subject,$content,$mailfrom);
    var_dump($res);
    $arrtest['ddd'] = 'i';
    $arrtest['dod'] = 'o';
    //Log::warning($arrtest);
    //Log::warning('log level test');
    Model_Log::warn('test');
